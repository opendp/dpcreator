...
export default {
    ...
    getCurrentTerms() {
        ...
        return wrappedSession.get('/api/terms-of-access/')
            .then(resp => {
     ...
     */
    getTermsOfUseLog() {
        return session.get('/api/terms-of-access-agreement/')
            .catch(resp => {
    ...
    acceptTermsOfUse(user, termsOfAccess) {
        return wrappedSession.post('/api/terms-of-access-agreement/', {user: user, terms_of_access: termsOfAccess})
    }
}