...
PARAMETERS_AND_BOUNDS_01 = [
    ...
    putil.txt_reg_para(
        f'This section briefly describes the parameters and bounds used within this report. For more in-depth information, please see the OpenDP website at https://opendp.org/about as well as documentation for the OpenDP library, https://docs.opendp.org.'),

    # Differentially Private (DP) Statistics
    putil.txt_bld_para_t10(f'Differentially Private (DP) Statistics'),
    putil.txt_reg_para_pl40(
        f'To make a DP release, we replace traditional methods to estimate statistics, or estimators, with randomized estimators that have a calibrated noise distribution.'),
    ...
    putil.txt_reg_para_pl40(
        f'Scale is amount of noise added to the query. If using the Laplace mechanism, Laplacian noise with variance 2 * scale^2 is added. Accuracy estimates are derived from this noise parameter.'),

    # Error
    putil.txt_bld_para_t10(f'Accuracy/Error'),
    putil.txt_reg_para_pl40(
        f'The accuracy or error is the greatest a released, noisy value may differ from the input to the noise mechanism, at a given confidence level. A key observation is that this is not the greatest the DP release may differ from the respective non-DP statistic.'),
...
PARAMETERS_AND_BOUNDS_02 = [
    ...
    putil.txt_reg_para_pl40(
        f'The probability that the epsilon bound fails. This is usually chosen to be very small, on the order of 1e-10. Some statistics and noise addition mechanisms require a delta parameter.'),
]
...
NEGATIVE_VALUES = [
    ...
    putil.txt_reg_para_pl40(
        f'The noise we add comes from a symmetric distribution about zero (discrete Laplace). This means it\'s just as likely to draw a noise value that is greater than zero than it is to draw a noise value less than zero.'),
    ...
    putil.txt_reg_para_pl40(
        'Adding symmetric noise doesn\'t introduce bias to the output. This simple approach also gives the most efficient estimator for counts--the lowest budget with the highest accuracy).'),
